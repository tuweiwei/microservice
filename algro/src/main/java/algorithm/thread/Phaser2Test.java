...
public class Phaser2Test {
    public static void main(String[] args) {
        ...
        for (char ch : name.toCharArray()){
            tourismThread.add(new Thread(new TourismRunnable2(phaser), "小" + ch));
        }
...
class TourismRunnable2 implements Runnable{
    Phaser phaser;
    Random random;
    /**
    ...
    public TourismRunnable2(Phaser phaser) {
        this.phaser = phaser;
    ...
    private boolean randomEvent() {
        ...
        if (r < 10){
            ...
            for (int i = 0; i < friendNum; i ++){
                new Thread(new TourismRunnable2(phaser), name + "的朋友" + frientCount.getAndAdd(1) + "号").start();
            }
        }else if(r > 90){
